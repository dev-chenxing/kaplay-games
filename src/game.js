...
export async function init() {
    ...
    // draw ground
    const ground = k.add(["ground", k.rect(k.width(), k.height() - sky_height), k.color(palette.light_green), k.outline(4, palette.black), k.pos(k.vec2(0, sky_height)), k.z(-100)]);
    const sprites = [];
    for (let i = 0; i < 75; i++) {
        sprites.push({
            sprite: "spike_grass",
    ...
    // npcs
    const bean = ground.add(["bean", k.sprite("bean"), k.pos(410, 190), k.area(), k.body()]);
}

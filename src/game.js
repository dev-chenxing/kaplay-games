...
export async function init() {
  ...
  k.addLevel(
    ...
    ],
    {
      ...
      tiles: {
        ...
        "-": () => [k.sprite("dirt")],
        "|": () => [k.rect(64, 64), k.area(), k.body({ isStatic: true }), k.opacity(0)]
      }
    }
  );
  ...
  // npcs
  const bag = k.add(["bag", k.sprite("bag"), k.pos(400, 600), k.area(), k.body(), k.state("idle", ["idle", "wander"]), { orientation: k.vec2(0, 0), speed: 50, isTalking: false, type: "npc" }]);
  ...
  function interact() {
    for (const col of player.getCollisions()) {
      ...
      console.log(target);
      if (target.activate) target.activate(target);
      if (target.type == "npc") {
        if (textbox.hidden) activateDialog(target);
      }
    }
    if (!textbox.hidden) deactivateDialog();
  }
